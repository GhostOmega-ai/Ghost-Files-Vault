...
export function canPreviewAsText(file) {
  const type = file.type || "";
  return type.startsWith("text/") || TEXT_EXTENSIONS.has(extensionOf(file.name));
}
...
export async function renderPreview(container, fileRecord) {
  ...
  if (type === "application/pdf" || extensionOf(name) === "pdf") {
    ...
    const objectUrl = createObjectUrl(container, blob);
    frame.src = `${objectUrl}#toolbar=0&navpanes=0&scrollbar=0&view=FitH`;
    frame.title = name;
...
function renderFallback(container, fileRecord) {
  container.dataset.previewKind = "fallback";

  const wrapper = document.createElement("div");
  wrapper.className = "viewer__fallback";
  wrapper.innerHTML = `
    <div class="viewer__fallback-card">
      <span class="viewer__fallback-icon" aria-hidden="true">${fileIcon(fileRecord)}</span>
      <strong>Stored safely in Ghost</strong>
      <span>${fileTypeLabel(fileRecord)} preview support is coming in the document viewer update. You can still pin, move, download or delete this file.</span>
    </div>
  `;
