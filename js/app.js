...
function bindEvents() {
  ...
  elements.backButton.addEventListener("click", closeAlbum);
...
function renderStorage() {
  ...
  elements.storageSummary.textContent = `${state.files.length} file${state.files.length === 1 ? "" : "s"} · ${formatBytes(totalBytes)}`;

  // Browser storage quotas differ; this bar is a visual activity indicator, not a quota claim.
  const visualPercent = Math.min(100, Math.max(0, totalBytes / (250 * 1024 * 1024) * 100));
  elements.storageBar.style.width = `${visualPercent}%`;
}
...
init().catch(error => {
  console.error(error);
  showToast("Ghost Files Vault could not start");
});
